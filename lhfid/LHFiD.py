...
class LHFID(GeneticAlgorithm):
    ...
    def __init__(self,
        ...
        set_if_none(kwargs, 'crossover', SBX(prob=0.9, eta=20))
        set_if_none(kwargs, 'mutation', PM(prob=0.1, eta=20))
    ...
    def _initialize(self):
        ...
        print("# LHFiD initialized successfully") 
...
def survival_selection(self):
    ...
    if self.nadir_point is not None:
        F_norm = normalize(F, xl=self.ideal_point, xu=self.nadir_point)
    else:
...
def update_termination_archives(self,Q):
    ...
    if len(P)>0 and len(Q)>0:
        if self.nadir_point is not None:
            P_norm = normalize(P, xl=self.ideal_point, xu=self.nadir_point)
            Q_norm = normalize(Q, xl=self.ideal_point, xu=self.nadir_point)
        else:
